...
export default function WorkshopPane({step,currentSubNum,data,track,busy,problems,hatch,openSubNum,actions}:StepPaneProps){
  return <div className="mw-pane mw-core-pane">
    <p className="mw-workshop-time">{step.time} · build session</p><h2 className="t20 mw-pane-title">{step.title}</h2><p className="mw-workshop-outcome">{step.outcome}</p>
    {step.subs.map(sub=>{
      ...
      return <article key={sub.num} className="mw-now"><p className="mw-activity-label">Activity {sub.num}</p><h3 className="mw-now-title">{sub.title}</h3><p className="c14">{sub.body}</p>
        {sub.code && <pre className="mw-lesson-code"><Code code={sub.code}/></pre>}
        {sub.fill && <CodingLesson key={sub.num} sub={sub} data={data} actions={actions} busy={busy===sub.num}/>}
    ...
    <a className="mw-reference-link" href="/?view=reference" target="_blank" rel="noopener noreferrer">Explore the completed reference laboratory ↗</a>
  </div>;
}
